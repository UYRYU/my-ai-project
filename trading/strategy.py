...
def generate_signals(
    ...
    for i in range(1, len(df) - 1):  # エントリーは次足なので最後の足は除外
        ...
        # --- リテスト + パターン判定 ---
        if state == "above":
            # 終値は EMA より上を維持している必要がある
            if curr_close <= curr_ema:
                state = None  # ブレイク失敗 → リセット
                continue
            # EMA にタッチ (安値が EMA を下回るか触れた)
            ...
            if touched and bullish_entry_pattern(o_arr[i], h_arr[i], l_arr[i], c_arr[i]):
                ...
                # 利確=直近高値, 損切=直近安値
                # エントリー価格に対して利確が上 / 損切が下 の場合のみ採用
                if recent_high > entry_price > recent_low:
                    signals.append(
                        Signal(
                            index=i,
                            side="long",
